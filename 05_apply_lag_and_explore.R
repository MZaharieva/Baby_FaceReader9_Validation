...
BFR_OBXT_valid_data <- BFR_OBXT_valid_data %>%
  ...
  dplyr::mutate(head_rotation = pmax(yaw, pitch, na.rm = TRUE)) %>%
...
# # Save and read data file with head rotations
#saveRDS(BFR_OBXT_valid_data, file = paste0("output/", analysis_type, "/BFR_OBXT_valid_data_head_rotations.rds"))
BFR_OBXT_valid_data <- readRDS(file = paste0("output/", analysis_type, "/BFR_OBXT_valid_data_head_rotations.rds"))
...
BFR_OBXT_valid_data %>% 
  ...
  ggridges::geom_density_ridges_gradient(scale = 0.95, 
                                         ...
                                         quantiles = 2) +
  theme_ridges() + 
  scale_fill_viridis_c(name = "Automatically Detected Global Emotional Valence", option = "D") +
  labs(title = 'Automatically Detected Global Emotional Valence Distribution', 
       x = 'Automatically Detected Valence',  y = 'Manually Coded Facial Expression Category') 

## Age ----

# Figure S4: Automatic Valence Distributions per Manually Coded Facial 
...
automatic_valence_age <- BFR_OBXT_valid_data %>%
  dplyr::mutate(age = dplyr::recode_factor(age, 
                                           `4` = "at 4 Months", 
                                           `8` = "at 8 Months"),
                'Automatically Detected Valence' = factor(paste(FE_category, age), 
  ...
  ggridges::geom_density_ridges(aes(x = valence, fill = `Automatically Detected Valence`),
                                scale = 1.5, 
                                quantile_lines = TRUE, 
                                quantiles = 2, 
                                panel_scaling = FALSE) +
  ggridges::theme_ridges() + 
  ggplot2::scale_fill_viridis_d(name = "Automatically Detected \nValence", 
                       option = "D", alpha = 0.7, 
                       direction = -1) +
  ggplot2::labs(title = 'Automatically Detected Global Emotional Valence \nper Manually Coded Facial Expression', 
       x = 'Automatically Detected Valence',  y = 'Manually Coded Facial Expression') + 
  ggplot2::xlim(-1, 1) +
...
# Figure S5: Automatic Valence Distribution per Age and Interaction partner.
...
automatic_valence_int_partner_age <- BFR_OBXT_valid_data %>%
  dplyr::mutate(age = dplyr::recode_factor(age, 
         ...
         'Automatically Detected Valence' = factor(paste(FE_category, int_partner), 
...
#                                                        "Negative with Unfamiliar Adult"))) %>%
  ...
  ggplot2::scale_fill_viridis_d(name = "Automatically Detected \nValence", 
                       option = "D", alpha = 0.5, 
                       direction = -1) +
  ggplot2::labs(title = 'Automatically Detected Global Emotional Valence \nper Age and Interaction Partner', 
       x = 'Automatically Detected Valence',  y = 'Manually Coded Facial Expression') + 
  ggplot2::xlim(-1, 1) +
  ggplot2::theme(panel.spacing = unit(0.5, "lines"), 
        strip.text.x = element_text(size = 14),
...
# Face Model Fit Certainty per Facial Expression Category. 
...
# Figure 2: Face Model Fit Certainty per Age.. 
vq_age <- BFR_OBXT_valid_data %>%
  dplyr::mutate(age = dplyr::recode_factor(age, `4` = "at 4 Months", `8` = "at 8 Months"),
         'Face Model Fit Certainty' = base::factor(base::paste(FE_category, age), levels = c(
  ...
  #dplyr::filter(FE_category != "Not Visible") %>%
  ggplot2::ggplot(., aes(y = FE_category)) + 
  ggridges::geom_density_ridges(aes(x = video_quality, fill = `Face Model Fit Certainty`),
                                scale = 1.5, 
                                quantile_lines = TRUE, 
                                quantiles = 2, 
                                panel_scaling = FALSE) +
  ggridges::theme_ridges() + 
  ggplot2::scale_fill_viridis_d(name = "Face Model Fit Certainty", 
                       option = "D", alpha = 0.7, direction = -1) +
  ggplot2::labs(title = 'Face Model Fit Certainty Distribution per Facial Expression Category', 
       x = 'Face Model Fit Certainty',  y = 'Facial Expression Category') + 
  ggplot2::xlim(0.4, 1) +
  #facet_wrap(~age, scales = "free_x", strip.position = "bottom") + 
  ggplot2::theme(panel.spacing = unit(0.5, "lines"), 
        strip.text.x = element_text(size = 14),
...
readr::write_excel_csv2(BFR_OBXT_head_rotation_summary, 
                         file = paste0("output/", analysis_type, "/head_rotation_summary_age_fe.csv"), 
                         na = "NA")
...
# Save plot.
ggplot2::ggsave(file = paste0("rplots/", analysis_type, "/head_rotations/head_rotations_age.jpg"), 
...
# Save plot.
ggplot2::ggsave(file = paste0("rplots/", analysis_type, "/head_rotations/pitch_age.jpg"), 
...
# Save plot.
ggplot2::ggsave(file = paste0("rplots/", analysis_type, "/head_rotations/yaw_age.jpg"), 
...
# Save plot.
ggplot2::ggsave(file = paste0("rplots/", analysis_type, "/head_rotations/roll_age.jpg"), 
